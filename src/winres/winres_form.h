...
protected:
    ...
    void AddSiblings(Node* parent_sizer, std::vector<std::reference_wrapper<resCtrl>>& actrls,
                     resCtrl* pSibling = nullptr);
    ...
    void CollectGroupControls(std::vector<std::reference_wrapper<resCtrl>>& group_ctrls,
                              size_t idx_parent);
    ...
    // of columns required.
    int GroupGridSizerNeeded(std::vector<std::reference_wrapper<resCtrl>>& group_ctrls,
    ...
    bool is_within_vertical(const std::vector<resCtrl>& ctrls, size_t child_a,
                            size_t child_b) const;

    // Returns true if child_a top/bottom is within child_b top/bottom. Only works with
    // master vector of controls (m_ctrls)
    bool is_within_vertical(const std::vector<std::reference_wrapper<resCtrl>>& ctrls,
    ...
    //
    // If loose_check == true, any control can be -2 of the top of the other control.
    bool is_same_top(const std::vector<resCtrl>& ctrls, size_t child_a, size_t child_b,
    ...
    //
    // If loose_check == true, any control can be -2 of the top of the other control.
    bool is_same_top(const std::vector<std::reference_wrapper<resCtrl>>& ctrls, size_t child_a,
                     size_t child_b, bool loose_check = false) const;
