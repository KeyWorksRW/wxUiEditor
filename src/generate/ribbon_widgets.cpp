...
wxObject* RibbonPageGenerator::Create(Node* node, wxObject* parent)
{
    ...
    auto widget = new wxRibbonPage((wxRibbonBar*) parent, wxID_ANY, node->GetPropertyAsString(txtLabel), bmp, 0);
...
{
    auto widget =
        new wxRibbonPanel((wxRibbonPage*) parent, wxID_ANY, node->GetPropertyAsString(txtLabel),
...
void RibbonButtonBarGenerator::AfterCreation(wxObject* wxobject, wxWindow* /*wxparent*/)
{
    ...
    for (size_t i = 0; i < count; ++i)
    {
        ...
        btn_bar->AddButton(wxID_ANY, childObj->GetPropertyAsString(txtLabel), bmp, childObj->GetPropertyAsString("help"),
