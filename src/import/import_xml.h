...
public:
    ...
    [[nodiscard]] virtual GenLang GetLanguage() const { return m_language; }
...
protected:
    ...
    [[nodiscard]] GenEnum::PropName MapPropName(std::string_view name) const;

    // Returns gen_unknown if the property name has not equivalent in wxUiEditor
    [[nodiscard]] GenEnum::GenName MapClassName(std::string_view name) const;
