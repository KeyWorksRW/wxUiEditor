...
#include "ttwx.h"  // Always include this first to avoid duplicate get_View() definitions
...
namespace
{
    ...
    auto trim_right(std::string_view str, size_t start, size_t end) -> size_t
    {
        ...
        while (temp_end > start && ttwx::is_whitespace(str.at(temp_end - 1)))
        {
            --temp_end;
        }
        return temp_end;
...
auto ttwx::ViewVector::ReadFile(std::string_view filename) -> bool
{
    ...
    auto file_size = std::filesystem::file_size(m_filename.ToStdString());
    ...
    file.read(m_buffer.data(), static_cast<std::streamsize>(file_size));
