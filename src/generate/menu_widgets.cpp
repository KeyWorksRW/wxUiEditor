...
wxObject* MenuBarBase::Create(Node* node, wxObject* parent)
{
    ...
    {
        auto label = new wxStaticText(panel, wxID_ANY, iter->prop_as_wxString(txtLabel));
...
void MenuBarBase::OnLeftMenuClick(wxMouseEvent& event)
{
    ...
    {
        if (m_node_menubar->GetChild(pos_menu)->prop_as_string(txtLabel) == text)
        {
...
wxMenu* MenuBarBase::MakeSubMenu(Node* menu_node)
{
    ...
    for (auto& menu_item: menu_node->GetChildNodePtrs())
    {
        if (menu_item->GetNodeTypeName() == "submenu")
        {
            ...
            auto item = sub_menu->AppendSubMenu(result, menu_item->GetPropertyAsString(txtLabel));
        ...
        {
            auto menu_label = menu_item->prop_as_string(txtLabel);
