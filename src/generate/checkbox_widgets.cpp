...
{
    long style_value = 0;
    if (node->prop_as_string(txtStyle).contains("wxALIGN_RIGHT"))
        style_value |= wxALIGN_RIGHT;

    auto widget = new wxCheckBox(wxStaticCast(parent, wxWindow), wxID_ANY, node->prop_as_wxString(txtLabel),
...
bool CheckBoxGenerator::OnPropertyChange(wxObject* widget, Node* node, NodeProperty* prop)
{
    ...
    {
        wxStaticCast(widget, wxCheckBox)->SetLabel(node->prop_as_wxString(txtLabel));
        return true;
    }
...
{
    long style_value = wxCHK_3STATE | node->prop_as_int(txtStyle) | node->prop_as_int("window_style");
