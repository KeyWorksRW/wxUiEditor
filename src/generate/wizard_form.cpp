...
std::optional<ttlib::cstr> WizardFormGenerator::GenCode(const std::string& cmd, Node* node)
{
    ...
    if (cmd == "ctor_declare")
    {
        ...
        code << node->prop_as_string(txtClassName) << "(wxWindow* parent, wxWindowID id = " << node->prop_as_string("id");
    ...
    else if (cmd == "after_addchild")
    {
        ...
        if (panes.size())
        {
            if (panes.size() > 1)
            {
                code << "    " << panes[0]->prop_as_string(txtVarName) << "->Chain(" << panes[1]->prop_as_string(txtVarName)
                     << ")";
                for (size_t pos = 1; pos + 1 < panes.size(); ++pos)
                {
                    code << ".Chain(" << panes[pos + 1]->prop_as_string(txtVarName) << ")";
                }
                code << ";\n";
            }
            code << "    GetPageAreaSizer()->Add(" << panes[0]->prop_as_string(txtVarName) << ");\n";
        }
...
std::optional<ttlib::cstr> WizardPageGenerator::GenConstruction(Node* node)
{
    ...
    if (node->IsLocal())
        code << "auto ";
    code << node->prop_as_string(txtVarName) << " = new wxWizardPageSimple(this";
