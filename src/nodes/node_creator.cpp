...
NodeCreator& NodeCreation = NodeCreator::get_Instance();

using namespace GenEnum;

NodeDeclaration* NodeCreator::get_NodeDeclaration(tt_string_view className)
{
...
size_t NodeCreator::CountChildrenWithSameType(Node* parent, GenType type) const
{
    ...
    for (const auto& child: parent->get_ChildNodePtrs())
    {
        ...
        else if (type == type_sizer && child->get_GenType() == type_gbsizer)
            ++count;
        else if (type == type_gbsizer && child->get_GenType() == type_sizer)
            ++count;
    }
                                                      ...
                                                      bool verify_language_support)
{
    ...
    {
        if (node_decl->is_Type(type_sizer))
        {
        ...
        else if (node_decl->is_Type(type_gbsizer))
        {
            auto count = CountChildrenWithSameType(parent, node_decl->get_GenType());
            if (count < (to_size_t) max_children)
            {
...
NodeSharedPtr NodeCreator::MakeCopy(Node* node, Node* parent)
{
    ...
    {
        if (auto copyProp = copyObj->get_PropPtr(iter.get_name()); copyProp)
        {
    ...
    if (parent && !parent->is_Gen(gen_Project) && node->is_Form())
    {
        ...
        if (child_object)
        {
            for (auto& iter: node->get_PropsVector())
            {
                auto copyProp = child_object->get_PropPtr(iter.get_name());
...
#if 0
    auto count = node->get_EventCount();
    for (size_t i = 0; i < count; i++)
    {
        auto event = node->get_Event(i);
        auto copyEvent = copyObj->get_Event(event->get_name());
        ASSERT(copyEvent);
        if (copyEvent)
