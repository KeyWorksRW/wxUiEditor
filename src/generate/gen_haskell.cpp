...
static bool GenerateHaskellForm(Node* form, GenResults& results, std::vector<tt_string>* pClassList = nullptr);
...
void MainFrame::OnGenSingleHaskell(wxCommandEvent& WXUNUSED(event))
{
    ...
    GenResults results;
    GenerateHaskellForm(form, results);
    ...
    wxMessageBox(msg, "Haskell Code Generation", wxOK | wxICON_INFORMATION);
}

void MainFrame::OnGenerateHaskell(wxCommandEvent& WXUNUSED(event))
{
    GenResults results;
    GenerateHaskellFiles(results);
...
static bool GenerateHaskellForm(Node* form, GenResults& results, std::vector<tt_string>* pClassList)
{
    auto [path, has_base_file] = Project.GetOutputPath(form, GEN_LANG_HASKELL);
    if (!has_base_file)
    {
#if !defined(_DEBUG)
        ...
        // :-)
        results.msgs.emplace_back() << "No Haskell filename specified for " << form->as_string(prop_class_name) << '\n';
#endif  // _DEBUG
        return false;
    }
    BaseCodeGenerator codegen(GEN_LANG_HASKELL, form);
    ...
    auto retval = cpp_cw->WriteFile(GEN_LANG_HASKELL, flags);
...
bool GenerateHaskellFiles(GenResults& results, std::vector<tt_string>* pClassList)
{
