...
static bool GenerateFortranForm(Node* form, GenResults& results, std::vector<tt_string>* pClassList = nullptr);

// clang-format off

constexpr const auto txt_Fortran_CmtBlock =
R"===(!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
void MainFrame::OnGenSingleFortran(wxCommandEvent& WXUNUSED(event))
{
    ...
    GenResults results;
    GenerateFortranForm(form, results);
    ...
    wxMessageBox(msg, "Fortran Code Generation", wxOK | wxICON_INFORMATION);
}

void MainFrame::OnGenerateFortran(wxCommandEvent& WXUNUSED(event))
{
    GenResults results;
    GenerateFortranFiles(results);
...
static bool GenerateFortranForm(Node* form, GenResults& results, std::vector<tt_string>* pClassList)
{
    auto [path, has_base_file] = Project.GetOutputPath(form, GEN_LANG_FORTRAN);
    if (!has_base_file)
    {
#if !defined(_DEBUG)
        // For a lot of wxRust testing of projects with multiple dialogs, there may only be a
        // few forms where wxRust generation is being tested, so don't nag in Debug builds.
        // :-)
        results.msgs.emplace_back() << "No Rust filename specified for " << form->as_string(prop_class_name) << '\n';
#endif  // _DEBUG
        return false;
    }
    BaseCodeGenerator codegen(GEN_LANG_FORTRAN, form);

    auto h_cw = std::make_unique<FileCodeWriter>(path);
    codegen.SetHdrWriteCode(h_cw.get());

    path.replace_extension(".rs");
...
bool GenerateFortranFiles(GenResults& results, std::vector<tt_string>* pClassList)
{
    ...
    {
        GenerateFortranForm(form, results, pClassList);
        if (results.updated_files.size())
        {
