...
int WriteCMakeFile(Node* parent_node, GenResults& results, int flag)
{
    ...
    {
        wxue::string new_dir(cmake_file);
        new_dir.remove_filename();
        wxSetWorkingDirectory(new_dir.wx());
    }
    else
    {
        Project.get_ProjectPath().ChangeDir();
    }
    if (cmake_file.find('.') == wxue::npos)
    {
        cmake_file << ".cmake";
    }

    wxue::string cur_dir(wxGetCwd().utf8_string());
    ...
    out.emplace_back();
    wxue::string var_name(Project.as_string(prop_cmake_varname));
    ...
    {
        std::set<wxue::string> base_files;
