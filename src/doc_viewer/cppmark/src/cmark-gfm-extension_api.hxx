// Extension API for creating custom markdown syntax
// Purpose: Allows extending markdown with new syntax (tables, strikethrough, task lists, autolinks,
// etc.) Key types: cmark_syntax_extension (hook functions), function typedefs for parsing/rendering
// callbacks Hook flow: Parser encounters line -> calls extension's match/open functions ->
// extension creates custom AST nodes -> renderer calls extension's render function Use case:
// Implement GFM tables, task lists, etc. without modifying core parser
...
struct subject;
using cmark_inline_parser = subject;
 ...
 */
typedef cmark_node* (*cmark_open_block_func)(cmark_syntax_extension* extension, int indented,
                                             cmark_parser* parser, cmark_node* parent_container,
 ...
 *  'false' otherwise.
 */
typedef int (*cmark_match_block_func)(cmark_syntax_extension* extension, cmark_parser* parser,
                                      unsigned char* input, int len, cmark_node* container);

typedef const char* (*cmark_get_type_string_func)(cmark_syntax_extension* extension,
                                                  cmark_node* node);

typedef int (*cmark_can_contain_func)(cmark_syntax_extension* extension, cmark_node* node,
                                      cmark_node_type child);

typedef int (*cmark_contains_inlines_func)(cmark_syntax_extension* extension, cmark_node* node);

typedef void (*cmark_common_render_func)(cmark_syntax_extension* extension,
                                         struct cmark_renderer* renderer, cmark_node* node,
                                         cmark_event_type ev_type, int options);

typedef int (*cmark_commonmark_escape_func)(cmark_syntax_extension* extension, cmark_node* node,
                                            int c);

typedef const char* (*cmark_xml_attr_func)(cmark_syntax_extension* extension, cmark_node* node);

typedef void (*cmark_html_render_func)(cmark_syntax_extension* extension,
                                       struct cmark_html_renderer* renderer, cmark_node* node,
                                       cmark_event_type ev_type, int options);

typedef int (*cmark_html_filter_func)(cmark_syntax_extension* extension, const unsigned char* tag,
                                      size_t tag_len);

typedef cmark_node* (*cmark_postprocess_func)(cmark_syntax_extension* extension,
                                              cmark_parser* parser, cmark_node* root);

typedef bool (*cmark_ispunct_func)(char c);

typedef void (*cmark_opaque_alloc_func)(cmark_syntax_extension* extension, cmark_node* node);

typedef void (*cmark_opaque_free_func)(cmark_syntax_extension* extension, cmark_node* node);
...
// carriage return (\r) trailing characters.
 ...
 *
 * Return 1 if the type could be changed, 0 otherwise.
 */
int cmark_node_set_type(cmark_node* node, cmark_node_type type);

// Return the string content for all types of 'node'. The pointer stays valid as long as 'node'
// isn't freed.
...
// Get the syntax extension responsible for the creation of 'node'. Return NULL if 'node' was
// created because it matched standard syntax rules.
...
// Gets the cmark_chunk being operated on by the given inline parser. Use
// cmark_inline_parser_get_offset to get our current position in the chunk.
struct cmark_chunk* cmark_inline_parser_get_chunk(cmark_inline_parser* parser);

// Returns 1 if the inline parser is currently in a bracket; pass 1 for 'image' if you want to know
// about an image-type bracket, 0 for link-type. */
int cmark_inline_parser_in_bracket(cmark_inline_parser* parser, int image);

// Remove the last n characters from the last child of the given node. This only works where all n
// characters are in the single last child, and the last child is CMARK_NODE_TEXT.
...
void cmark_inline_parser_push_delimiter(cmark_inline_parser* parser, unsigned char c, int can_open,
